...
public class SearchModel extends BaseViewModel {
    ...
    void getAllCities() {
        CoreManager.getDefault().getImpl(ICoreExecutors.class).diskIOThread().execute(new Runnable() {
            @Override
            public void run() {
                List<City> allCity = CoreManager.getDefault().getImpl(ICityProvider.class).queryAllCities();
                if (allCity != null) {
    ...
    void matchCities(final String key) {
        CoreManager.getDefault().getImpl(ICoreExecutors.class).diskIOThread().execute(new Runnable() {
            @Override
            public void run() {
                List<City> allCity = CoreManager.getDefault().getImpl(ICityProvider.class).matchingCity(key);
                if (allCity != null) {
