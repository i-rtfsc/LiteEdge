...
import com.journeyOS.plugins.key.adapter.KeyHolder;
...
public class KeySelectorFragment extends BaseFragment implements RouterListener {
    private static final String TAG = KeySelectorFragment.class.getSimpleName();
    ...
    public static Fragment newInstance(Activity activity, int rotation, FingerDirection direction) {
        KeySelectorFragment fragment = new KeySelectorFragment();
        sRotation = rotation;
    ...
    protected void initDataObserver(Bundle savedInstanceState) {
        ...
        mKeyModel = ModelProvider.getModel(this, KeyModel.class);
        mKeyModel.getKeyApps();

        mKeyModel.getAllKeyData().observe(this, new Observer<List<KeyInfoData>>() {
            @Override
            public void onChanged(@Nullable List<KeyInfoData> infoDatas) {
                onAllApps(infoDatas);
            }
        });
    ...
    void onAllApps(final List<KeyInfoData> appInfoData) {
        ...
        mAllAppsAdapter.registerHolder(KeyHolder.class, R.layout.layout_app_item);
    ...
    public void onShowMessage(RouterMsssage message) {
        ...
        switch (msg.what) {
            case Messages.MSG_ADD_GESTURE_KEY:
                KeyInfoData infoData = (KeyInfoData) msg.obj;
                save(infoData);
                break;
        }
    }

    public void save(final KeyInfoData infoData) {
